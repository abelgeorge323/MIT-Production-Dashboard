...
def load_active_roster():
    ...
    try:
        ...
        for skiprows in [0, 1, 2, 3, 4, 5]:
            try:
                ...
                if not df.empty and len(df.columns) > 3:
                    st.markdown(f'<div class="status-box">✅ Successfully loaded MIT candidate data! (skiprows={skiprows})</div>', unsafe_allow_html=True)
                    break
            except:
                continue
        else:
        ...
        st.markdown(f'<div class="status-box">📋 Available columns: {list(df.columns)}</div>', unsafe_allow_html=True)
        ...
        main_df = df.copy()
        entering_df = pd.DataFrame()
        
        # Filter out "Position Identified" candidates if Status column exists
        ...
        elif 'Start Date' in main_df.columns:
            main_df['Start Date'] = pd.to_datetime(main_df['Start Date'], errors='coerce')
        ...
        if 'Salary' in main_df.columns:
            main_df['Salary'] = main_df['Salary'].astype(str).str.replace('$', '').str.replace(',', '').str.replace(' ', '')
            main_df['Salary'] = pd.to_numeric(main_df['Salary'], errors='coerce')
        ...
        if 'VERT' in main_df.columns:
            main_df['Vertical Full'] = main_df['VERT'].map(vertical_map).fillna(main_df['VERT'])
    ...
    try:
        ...
        for skiprows in [0, 1, 2, 3, 4, 5, 6]:
            try:
                df = pd.read_csv(url, skiprows=skiprows)
                if not df.empty and len(df.columns) > 3:
                    st.markdown(f'<div class="status-box">✅ Successfully loaded job positions! (skiprows={skiprows})</div>', unsafe_allow_html=True)
                    break
            except:
                continue
        else:
